...
def mypage(request):
    ...
    selected = Select.objects.filter(user=request.user)
    return render(request, 'mypage.html', {"posts":posts ,'likes':likes, "selected":selected})
...
def select(request):
    if request.method == "POST":
        ...
        post_selected = Select.objects.filter(
            post = Post.objects.get(pk=post_pk),
            user=request.user
        )

        response = {
            'select_count': post_selected.count(),
        }
...
def checkSelect(request):
    if request.method =='POST':
        request_body = json.loads(request.body)
        post_pk = request_body['post_pk']
        existing = Select.objects.filter(
            post=Post.objects.get(pk=post_pk),
            user=request.user
        )
        response = {
